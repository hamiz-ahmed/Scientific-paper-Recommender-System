...
from .model_forms import AlgorithmsForm
...
def index(request):
    if request.user.is_authenticated:
        ...
        if model.SystemRecommendations.objects.filter(isactive=True).exists():
            ...
            Systemalgo = None
        response = render(request, 'GUIManager/home.html',
...
def register(request):
    if request.method == 'POST':
        ...
        form = UserForm()
        return render(request, 'GUIManager/register.html',
...
def search(request):
    if request.user.is_authenticated:
        if request.method == 'GET':
            ...
            paper_manager = PaperDataManager()
            papers_list = paper_manager.get_papers_for_query_from_db(
        ...
        if request.user.is_superuser:
            if request.method == 'POST':
                ...
                if form.is_valid():
                    ...
                    new_algorithm = Algorithm(name=name,
                                              description=description,
                                              created_by=created_by,
                                              in_use=True)
                    new_algorithm.save()
                    algo_db_wrapper = AlgorithmManager()
                    algo_db_wrapper.insert_algorithm(name=name,
                                                     description=description,
                                                     created_by=created_by,
                                                     file=request.FILES[
                                                         'file'],
...
def algorithms(request):
    if request.user.is_authenticated:
        ...
        return render(request, 'GUIManager/show_table.html',
                      {'table': table, 'name': 'Add algorithm',
                       'link': 'add_algorithm', 'show_button': True})
    else:
        return redirect('login')
            ...
            if request.user.is_superuser:
                if request.method == 'POST':
                    ...
                    if form.is_valid():
                        ...
                        description = form.cleaned_data['description']
                        created_by = request.user
                        new_name = "{}_{}".format(algorithm.name,
                                                  timezone.localtime(
                                                      timezone.now()))
                        ...
                        algo_manager.insert_algorithm(name=name,
                                                      description=description,
                                                      created_by=created_by,
                                                      file=request.FILES[
                                                          'file'],
...
def delete_algorithm(request, algorithm_id):
    if request.user.is_authenticated:
        ...
        if request.user.username == algorithm.created_by.username:
            ...
            algorithm.name = "{}_{}".format(algorithm.name,
        ...
        if request.method == "POST":
            ...
            response = render(request,
                              ...
                              {'recommendations': recom,
                               'algo': 'ScipR System' if
                               is_system_rec is not None else algo.name,
...
def eval_results(request):
    ...
    p10_data = [0 if not precision[i][0] else precision[i][0] for i in
                precision_label]
    p20_data = [0 if not precision[i][1] else precision[i][1] for i in
                precision_label]
    p30_data = [0 if not precision[i][2] else precision[i][2] for i in
                precision_label]
    p40_data = [0 if not precision[i][3] else precision[i][3] for i in
                precision_label]
    p50_data = [0 if not precision[i][4] else precision[i][4] for i in
                precision_label]
...
def show_feedback_rating(request):
    if request.user.is_authenticated:
        ...
        response = render(request,
                          ...
                          {'ratings': ratings,
                           'all_algorithms': AlgorithmManager().
                          get_available_algorithms(),
                           'past_system_algo': model.
                          SystemRecommendations.objects.all(),
                           'ratingmatrix': model.
                          RatingMatrix.objects.count()}
                          )
        return response
...
def update_system_recommendation(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            h.set_system_recommendation(request.user,
                                        request.POST.get('algo_id'),
            ...
            response = render(request,
                              ...
                              {'ratings': ratings,
                               ...
                               'past_system_algo': model.
                              SystemRecommendations.objects.all(),
                               'Success': "Your changes were updated "
                                          "succesfully"
                               }
                              )
