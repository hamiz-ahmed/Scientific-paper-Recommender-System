...
class Metrics:
    ...
    def __init__(self, query):
        ...
        def ground_truth_of_user(user_query):
            ...
            relevant_papers = model.UserFeedback.objects \
                .filter(
                recommendationresult_detail__recommendation__id=user_query.id,
                user_rating=1,
                feedback_type='explicit')
            ground_truth_list = ground_truth_list + [
                i.recommendationresult_detail.paper.id for i in
                relevant_papers]
            # TODO : COnsult if catalog has to be considerd as ground truth,
            #  if yes uncomment below line
            ''' ground_truth_list = ground_truth_list + [i.id for i 
            in user_catalog] '''
    ...
    def NDCG(self, pages):
        ...
        max_dcg = 1 + math.fsum( \
            [1 / math.log2(i + 1) for i in
             range(1, len(self.ground_truth) - 1)])
