...
def read_mult(doc_vocab_file, with_header=False, lda_c_format=True,
              delimiter=' '):
    """
    Code from Anas
        ...
        for line in f:
            ...
            for e in line_split[start:]:
                ...
                try:
                    ...
                    print(
                        "Error in doc_vocab file {} : line {}, value {} "
                        "is not int.".format(
                            doc_vocab_file, line_num,
                            e))
                    raise
    ...
    if len(vocabs) != max(vocabs) + 1:
        print(
            "Error in doc_vocab file {}: # Vocabs = {}, max vocab = {}".format(
                doc_vocab_file, len(vocabs),
                max(vocabs)))
        raise ValueError
...
def fit_lda():
    ...
    This function was executed in another server and the model file
    transferred to Django
    :return:
    """
    print('file reading started at ', time.ctime())

    doc_matrix = read_mult('Helpers/mult.dat')
...
def save_model_to_file(model):
    """
    saves the fitted model as a file and inserts it into the django/system
    cache

    :param model:
    :return:
    """
    filename = 'model' + time.ctime().replace(':', '').replace(' ',
                                                               '') + '.dat'
    try:
...
def get_LDA_topics(paper_obj):
    ...
    if cache.get('lda_model') is None:
        ...
        topic_array = cache.get('lda_model').transform(
            get_paper_vocabulary(paper_obj))
    # return a dictionary of topic_id and those values greater than .08
    return {i: v for i, v in enumerate(topic_array[0].tolist()) if
            v > THRESHOLD}
