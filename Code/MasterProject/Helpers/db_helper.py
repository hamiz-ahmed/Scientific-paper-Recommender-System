...
def get_feedback_ratings():
    ...
    return model.Algorithm.objects.annotate(
        ratings=Count("recommendation__recommendationresult__userfeedback"))


def get_metrics_systemrecommendations(metric_name):
    if metric_name == "precision":
        # Build an annotated queryset
        # p will contain a list of objects with property algorithm name and
        # p_10,p_20 etc
        p = model.SystemAlgorithmRecommendationLog.objects.annotate(
            p_10=Avg("recommendation__evaluationmetric__p_at_10"),
        ...
        precision = {}
        for i in p:
            key = i.system_algorithm.algorithm.name + '-' + str(
                i.system_algorithm.id)
        ...
        p = model.SystemAlgorithmRecommendationLog.objects.annotate(
            mrr_10=Avg("recommendation__evaluationmetric__mrr_at_10"),
        ...
        p = model.SystemAlgorithmRecommendationLog.objects.annotate(
            ndcg_10=Avg("recommendation__evaluationmetric__ndcg_at_10"),
        ...
        p = model.SystemAlgorithmRecommendationLog.objects.annotate(
            ctr=Avg("recommendation__evaluationmetric__ctr"))
        ctr = {}
        for i in p:
            key = i.system_algorithm.algorithm.name + '-' + str(
                i.system_algorithm.id)
...
def get_metrics(metric_name):
    if metric_name == "precision":
        # Build an annotated queryset
        # p will contain a list of objects with property algorithm name
        # and p_10,p_20 etc
        p = model.Algorithm.objects.annotate(
            p_10=Avg("recommendation__evaluationmetric__p_at_10"),
            p_20=Avg("recommendation__evaluationmetric__p_at_20"),
            p_30=Avg("recommendation__evaluationmetric__p_at_30"),
            p_40=Avg("recommendation__evaluationmetric__p_at_40"),
...
def update_rating_matrix(user, paper, rating):
    """
        Saves the RatingMatrix Table when user updates his catalog through
        search.
        The function ensures only the latest rating is available for a
        user-paper combo
    ...
    user_mapping = user.usermapping_set.all()[
        0]  # getting only the first object since its a one to one mapping

    rm_obj = model.RatingMatrix.objects.filter(user_map=user_mapping,
                                               paper=paper)
    ...
    # else create a new entry
        ...
        return model.RatingMatrix.objects.create(
            user_map=user_mapping, paper=paper, doc_id=paper.doc_id,
            external_user_id=user_mapping.external_user_id, rating=rating)
...
def convert_feedback_rating():
    """
       tranfer the ratings in UserFeedback to Rating Matrix table
        :return: int
    """
    # Get all ratings from Userfeedback and insert or update them in
    # RAtingMATRIX
    feedback_items = model.UserFeedback.objects.values_list(
        'recommendationresult_detail__recommendation__user',
        'recommendationresult_detail__paper',
        'user_rating',
        'datetime')
    for i in feedback_items:
        user_map = model.UserMapping.objects.get(user_id=i[0])
        obj, created = model.RatingMatrix.objects.get_or_create(
            user_map=user_map,
            paper_id=i[1],
            defaults={'rating': i[2]})
        obj.rating = i[2]
...
def get_ratings(user_map=None):
    ...
    :param user_map (user model object)
    :return: QuerySet
    """
    if user_map is None:
        return model.RatingMatrix.objects.all()
    else:
        return [i.paper.doc_id for i in
                model.RatingMatrix.objects.filter(external_user_id=user_map,
                                                  rating=1)]
...
def get_title_like(title_string):
    return model.Paper.objects.filter(
        title__icontains=title_string).values_list('title', flat=True)
...
def get_model_paper_object(paper_dict):
    ...
    :param paper_dict:
    :type paper_dict: PaperMetaData
    :return:
    :rtype: DB_MODEL
    """
    paper_model = model.Paper(url=paper_dict['paper_url'],
...
def add_user(user_data):
    ...
    :rtype:
    """
    user = model.User(last_name=user_data.cleaned_data['last_name'],
                      first_name=user_data.cleaned_data['first_name'],
                      email=user_data.cleaned_data['email'],
                      username=user_data.cleaned_data['username'],
                      is_active=1,
                      is_superuser=user_data.cleaned_data['is_superuser'],
...
def add_user_mapping(user):
    ...
    max_user_id = model.UserMapping.objects.all().aggregate(
        Max('external_user_id'))['external_user_id__max']
    ...
    user_mapping = model.UserMapping(user=user,
