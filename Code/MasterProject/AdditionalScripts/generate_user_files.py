...
def generate_user_mapping_file(users_file_path, out_path):
    ...
    users_df = pd.read_csv(users_file_path, sep=',', quotechar='|',
    ...
    max_ext_user_id = \
    model.UserMapping.objects.all().aggregate(Max('external_user_id'))[
        'external_user_id__max']
    ...
    # save the df to csv
    final_df = pd.DataFrame({'id': ids, 'external_hash': hash,
                             'external_user_id': external_user_ids,
                             'user_id': user_ids})
    final_df.to_csv(out_path + 'usermap_db.csv',
                    columns=['id', 'external_hash', 'external_user_id',
                             'user_id'],
                    sep=';', line_terminator='\n', quotechar='|',
...
def generate_ratings_file(ratings_file, outpath):
    ...
    ratings_df = pd.read_csv(ratings_file, sep=',',
                             names=['citeulike_id', 'user_hash', 'timestamp',
                                    'tag'], index_col=False)

    # load user mapping file
    user_mapping_df = pd.read_csv(outpath + 'usermap_db.csv', sep=';',
                                  quotechar='|', quoting=csv.QUOTE_MINIMAL,
                                  dtype={'id': int, 'external_hash': str,
                                         'external_user_id': int,
                                         'user_id': int})

    # load paper data file
    papers_df = pd.read_csv(outpath + 'paper_meta_data_db.csv', sep=';',
                            quotechar='|', quoting=csv.QUOTE_ALL,
                            lineterminator='\n', index_col=False)

    # drop duplicates from it
    ratings_filtered_df = ratings_df[
        ['citeulike_id', 'user_hash', 'timestamp']].drop_duplicates()
    ...
    map_ratings_df = user_mapping_df.merge(ratings_filtered_df,
                                           on='external_hash', how='inner',
                                           suffixes=['_map', '_rating'])

    map_ratings_df['external_id'] = map_ratings_df['external_id'].astype(
        'int64')

    final_merged_df = map_ratings_df.merge(papers_df, on='external_id',
                                           how='inner',
                                           suffixes=['_mapping', '_paper'])
    ...
    final_df = pd.DataFrame(
        {'id': ids, 'rating': rating, 'timestamp': timestamp,
         'paper_id': paper_id,
         'user_map_id': user_map_id, 'doc_id': doc_ids,
         'external_user_id': external_user_ids})

    final_df.to_csv(outpath + 'ratings_db.csv',
                    columns=['id', 'rating', 'paper_id', 'timestamp',
                             'user_map_id', 'doc_id', 'external_user_id'],
                    sep=';', line_terminator='\n', quotechar='|',
                    quoting=csv.QUOTE_MINIMAL, index=False)
